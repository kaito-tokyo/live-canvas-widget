...
function Invoke-ObsConfigure {
    ...
    param(
        ...
        [string]$WindowsArch = $null,
        [string]$OsxArchitectures = $null,
        [string]$OsxDeploymentTarget = '12.0'
    )
    process {
        Set-StrictMode -Version Latest; $ErrorActionPreference = 'Stop'; $PSNativeCommandUseErrorActionPreference = $true; $ProgressPreference = 'SilentlyContinue'
        ...
        if ($IsWindows) {
            ...
            if ($WindowsArch) {
                $cmakeConfigureArgs += @('-A', $WindowsArch)
            }
            else {
            ...
            if ($WindowsSdkVersion) {
                $cmakeConfigureArgs += "-DCMAKE_SYSTEM_VERSION=$WindowsSdkVersion"
            }
        ...
        elseif ($IsMacOS) {
            if ($Generator) {
                ...
                $cmakeConfigureArgs += @('-G', 'Xcode')
            }

            if ($OsxArchitectures) {
                $cmakeConfigureArgs += "-DCMAKE_OSX_ARCHITECTURES=$OsxArchitectures"
            }

            if ($OsxDeploymentTarget) {
                $cmakeConfigureArgs += "-DCMAKE_OSX_DEPLOYMENT_TARGET=$OsxDeploymentTarget"
            }
        }
        else {
            throw 'Unsupported platform'
